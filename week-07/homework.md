...
5. **[Its own database](lecture-notes.md#naming-your-database)** — one per application, so **not** the `Cryptids_…` one you used in the lab. Name yours after this app: `YourAppName_<COURSE-NUMBER>_<YOUR-INITIALS>`, filled in — `TrailGuide_42_ABL`. It doesn't exist until your first `dotnet ef database update`, which creates it.
...
The report prints in the order a checker can *reach* things, crawling your site from the outside.
Each line is green before you start; what matters is which requirement turns it **red** if you break it:
...
**You installed the tag in Part 2, so it is already there.** Run your app **locally** (`dotnet watch`), load your home page, and open the console — **F12 → Console**. It prints one line and waits — nothing runs, and nothing is submitted, until you type **`recheck()`**. So reloading while you build never adds anything to your list. `recheck()` fetches fresh pages, so you don't need to reload first — change your code, let `dotnet watch` rebuild, then press Up and Enter in the console.
...
- **`No project was found in the current working directory`** — `dotnet ef` and `dotnet add package` both run from the folder with your `.csproj`, not the one above it. **The lab handed you the two EF packages; this is the first time you install them yourself**, so it's the first time this bites.
- **`Your startup project doesn't reference Microsoft.EntityFrameworkCore.Design`** — you added `.SqlServer` but not `.Design`. It's the one the `dotnet ef` tool needs; your app never calls it, which is why it's easy to skip. [Both commands are here](lecture-notes.md#two-packages).
- **`dotnet ef` isn't recognized as a command** — the tool installs once per *machine*, not per project: `dotnet tool install --global dotnet-ef`.
