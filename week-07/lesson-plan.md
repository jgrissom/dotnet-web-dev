...
- **The lab has 50 minutes and task 1 is a connection string.** Watch for anyone still fighting it at the ten-minute mark and go to them; it is the only task tonight that cannot be unblocked by reading. Everything after it is C#. The two most likely stalls are running `user-secrets` from the wrong folder (it needs `Cryptids.Web`, same as `dotnet ef`) and a typo in the key name — `dotnet user-secrets list` settles both in one command.
