...
- [ ] Stop `dotnet watch`. From **inside** `CommonGrounds.Web/`:
  ...
  ```
> [!IMPORTANT]
> **While it churns (~2–3 min), set the room going first, then narrate.** The beats below are in order, and **the command finishes when it finishes** — you stop wherever you've got to, mid-sentence if need be.
