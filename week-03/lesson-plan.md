...
- Terminal + VS Code (Teaching profile) ready; `~/Repos/dotnet-web-dev-course/instructor/week-03/` ready and empty — the demo app is created in it live
...
| 2:10 | 10 min | **☕ Break** |
| 2:20 | 40 min | **To the cloud** *(slides 14–15, demo §4)*. What App Service is; then you deploy the demo app **live** while students work [`deploy-guide.md`](deploy-guide.md) steps 1–2 (install `az`, login) on their own machines. End with the demo app's Azure URL on screen — and phones out, everyone loads it. **Start a visible "working regions" list** (board/shared doc) — region availability varies per subscription, and the class crowd-sources the good ones within a week. |
| 3:00 | 35 min | **Lab: First Flight** *(slide 16)*. Launch with *what done looks like* — your finished copy running + `dotnet test` printing **6 / 6** — then walk the room through the loop once on a copy of their own starter: two terminals, `Failed: 5, Passed: 1`, read check 2's failure, fix the brand, re-run. It's the first week they run two terminals at once, and the first red-to-green they see. **In-class target: checks 1–4 green** (brand, About, nav). Checks 5–6 and the deploy roll into homework by design — say so. |
...
- Demo from your **Teaching profile**; terminal font up for the projector. The demo app (`CommonGrounds.Web`) is created live in `~/Repos/dotnet-web-dev-course/instructor/week-03/` — different content from the lab on purpose.
