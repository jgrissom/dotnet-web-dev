...
| 0:10 | 15 min | **Where we left off** *(slides 2–3, demo §1)*. Restart, seven trucks, full CRUD — *"and every truck is still an island."* **The three words** on slide 3: principal, dependent, foreign key — name each one against `Truck` and `Special` rather than asking the room to recall it. Land the framing that isn't on the slide: principal and dependent are about **who can exist alone**, not importance. |
...
| 0:45 | 15 min | **The migration** *(slide 5, demo §3)*. No prediction here — nothing they've typed points at the answer. Name what you're going looking for, then read the four operations: `CreateTable`, the FK constraint, `InsertData`, and the **index nobody asked for**. Then the line that earns the slide: `onDelete: Cascade`, **inferred from a missing `?`**. **Collect week 8's reading question** here — what happens to the review when the trail goes. Apply, then mssql: fourteen rows. |
| 1:00 | 10 min | **☕ Break** |
| 1:10 | 15 min | **Failure #1 — the empty list** *(slide 6, demo §4)*. Render the specials on Details with **no `Include`**. Predict against a number they just saw in the mssql panel: *"Roll Models has two. What am I about to see?"* → **`Nothing listed.`** ⚠️ **Let it sit.** Walk the instruments out loud — 200, no error, no terminal line, rows in the database — *then* ask for the diagnosis. Slide 6 is the answer, not the setup. One line fixes it. |
| 1:25 | 20 min | **The hidden cost — how many SELECTs** *(slides 7–8, demo §5)*. Write the per-truck count loop the honest way. **Slide 7 is the exercise** — hands up for one, seven, eight. Reload: **eight**. Then `Include`: **one `LEFT JOIN`**. ⚠️ The rider is the exam question: **`Include` is per query**, and the one on Details did nothing for this page. |
| 1:45 | 25 min | **The ViewModel and the form** *(slides 9–11, demo §6–§7)*. Read the existing `(List<Truck>)ViewData["AlsoHere"]!` aloud — a cast **and** a `!` in a view. Build `ViewModels/`, ⚠️ **adding the `_ViewImports` using in the same breath** (a missing namespace breaks *every* view, and the errors point at files nobody touched). Then `SelectList`, the form, and **failure #2**: a perfectly filled form refused with *"The Trucks field is required"*. Reach for `asp-validation-summary="All"` first — week 6's advice, still the fastest move — then fix with **one `?`**. |
| 2:10 | 10 min | **Delete takes the children** *(slide 12, demo §8)*. Ask before clicking: *"three specials — what happens to them?"* Answer against the slide, then tie it back to the `int` typed in §2. Build the warning. 💡 **Fourth `Include` of the night** — Details, Index and the *Also in* list each needed their own; Delete still showed zero. |
...
- **Failure #2 rewards the debugging move before it rewards the fix.** The form fails silently under `ModelOnly`; switching to `asp-validation-summary="All"` is what makes the message appear. That ordering is the lesson — week 6's troubleshooting advice paying off — so don't jump straight to the `?`.
...
- 🔗 **Week 8's reading question gets collected tonight** — what happens to the review when the trail is deleted (§3, answered by `onDelete: Cascade`). That one was set a week ago and they can still reach it. ⚠️ **Don't reach further back than that.** Week 7 is two class sessions and a term break away; a callback to it collects silence, not recognition.
- **The lab's watch-for is task 4, not task 2.** Task 2 is a model edit and task 3 is a migration — both familiar by now. Task 4 is `Include`, and its failure mode is a page that looks finished and shows nothing. **Sweep the room at task 4**, and when someone says "it's not working," ask what their Details action looks like before you ask anything else.
- ⚠️ **The lab's task 2 deletes a column.** That is deliberate and it is the Registry's own story — `Sightings` was a number somebody typed. Their *homework* adds a table without removing anything, so say the difference out loud or a few people will go looking for something of their own to delete.
