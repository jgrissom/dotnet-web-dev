...
**🖥️ On screen, at curtain** — checklist above done, this is what the room walks in to; tonight is the browser, not the editor:
...
- [ ] ⚠️ **Step 6 is new and nobody will think it matters — it does.** It makes `dotnet-web` and clones the starters inside it. Every later week opens with `git -C dotnet-web-starters pull`, run from `dotnet-web` — which only works if the clone is sitting in there. *"One folder holds this whole course. Make it where you will find it again — not the Desktop, not Downloads"*
