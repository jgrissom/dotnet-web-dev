...
| 0:20 | 45 min | **Environment setup workshop.** Students install/verify: .NET 10 SDK (`dotnet --version`), VS Code with the **C#** extension (verify by scaffolding a throwaway project: `dotnet new console -o Hello`, opening the folder, and pressing F5 — breakpoint hits), the **SQL Server (mssql)** VS Code extension (add a connection to the school SQL Server with their own account — distribute server address + credentials handout), Git (`git --version`), and a GitHub account (school email — needed for tonight's homework). Mention Azure for Students exists but activation waits until week 3. Circulate and troubleshoot. Fast finishers help neighbors. |
| 1:05 | 10 min | **☕ Break** |
