...
> [!NOTE]
> **There is no lab, no `*.Checks` project and no starter to pull this week** — so nothing to `git pull` from `dotnet-web-starters`, and no `dotnet test`. A red-to-green ladder on a shared starter is the wrong instrument for a midterm that lives in fourteen different codebases. The session's hands-on time is the studio block, on their own apps.
...
- ⚠️ **Answer `a` at the first restart prompt.** §3 makes both edits hot reload cannot apply — a new `.cshtml`, and `Program.cs`, which only runs at startup
