...
| 1:00 | 10 min | **☕ Break** |
| 1:10 | 20 min | **A table nobody can add to** *(slides 7–8, demo §4)*. `/Dishes` is read-only from outside and nobody decided that. Build the Create pair, then **use it** — and the new dish's page is a heading with nothing under it. **Slide 7 asks the question; don't answer it before the browser does.** Then the `@if`, twice more by paste. |
...
- 🎯 **Push them to deploy before they leave.** Nothing about `az webapp up` needs to happen at home — they have deployed since week 3 — and a broken deploy is a five-minute problem with you present and an evening-ender without you. This is the same reasoning behind screening deploys early in week 7's session rather than at the end.
- ⚠️ **The studio IS the homework, and you have to say so.** There is no separate lab this week, so what they do in the room is the graded assignment on their own app — slide 10's bullets are homework requirements 1 and 3–7. Nine weeks have taught them that lab ≠ homework and that the lab is never collected; without a sentence naming the change, some will treat the studio as throwaway practice and start again from scratch at home.
...
- **One question, asked of everyone, however you are teaching:** *"what are your three, and which one is the worst?"* It gets a student off their editor and back onto the assignment faster than looking at their code does. In a room, circulate and ask it at each desk. **On Zoom, collect all of them in the chat before anyone starts working** — the whole class takes about ninety seconds, you get the set in writing rather than one at a time, and it doubles as your map of who is stuck. Then run a queue in the chat and take screen-shares in order. Breakout rooms are the wrong tool here: everyone is working alone, so they would only move students further from you.
- ⚠️ **Say that their three cannot be your three.** Requirements 1–4 are the floor and they are exactly what you demoed — front door, template pages, empty states, wrong-id page, 8 of the 20 points. Requirement 5 says plainly that their three *"are not on this list"*, but that is one clause in a document, and the demo is an hour of screen time. Without the sentence, a student writes down what you just showed them and the 4 points meant to measure their own judgement measure their memory instead.
