...
- **Only the CSS changed.** The `<script>` for `bootstrap.bundle.min.js` stays local and untouched, so everything Bootstrap does with JavaScript — dropdowns, modals, tooltips — still works. In this app the one you can see is the mobile navbar: narrow the window past the `sm` breakpoint and the toggler still opens.
