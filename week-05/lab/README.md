...
| # | Check | What to do |
|---|-------|------------|
| 2 | `ShellIsBranded` | The app is called **`Cryptid Registry`**, not `Cryptids.Web`. Change **three things** in `_Layout.cshtml`: the `navbar-brand` text, the suffix in the `<title>` line, and the footer — add **`Field Reports Since 1893`** to its text, word for word. *(Add, not replace — keep the © and the Privacy link if you like; only that phrase is checked.)* Then look at all three tabs: three edits, one file, and every page in the app changes at once — [that's the whole idea](../lecture-notes.md#branding-the-shell). |
...
- **`The partial view '_CryptidCard' was not found`** — it looks in this controller's view folder, then `Views/Shared/`. Check the file is in `Views/Shared/`, that the name in `<partial name="_CryptidCard" />` has the underscore, and that it does **not** have `.cshtml` on it. **Then check the spelling character for character** — `_CrytpidCard.cshtml` and `_CryptidCard.cshtml` are different files, and a transposed letter is invisible until you look for it. The error lists the paths it searched; hold them next to what's actually in `Views/Shared/`.
