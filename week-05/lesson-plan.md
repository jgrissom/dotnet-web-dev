...
4. Extract repeated markup into a partial view, render it from the layout and from a view, and pass it a model.
...
|------|----------|---------|
| 0:00 | 15 min | **Deployed-app gallery** *(deck on title slide)*. **First one — week 4 promised it.** 3–4 student apps on the projector, 2 minutes each, everyone's turn comes before term ends. Then collect last week's reading: *"two things in `_Layout.cshtml` you'd change site-wide."* **Write the answers on the board** — they are tonight's agenda, and you tick them off as the night covers them. Update the working-regions list. |
| 0:15 | 35 min | **The shell you've been ignoring** *(slides 3–6, demo §1)*. **Open live, not rhetorically:** their Index view in the editor (37 lines, no `<html>`) next to View Source on `/Trucks` (127 lines, their `<h1>` at line 40). *Who wrote the other 90?* Then read `_Layout.cshtml` top to bottom without editing. Then **slide 5 is live**: predict, then delete `@RenderBody()` — it's a **500**, not a blank page, and the exception names itself. **Restore and verify.** Then the title's two halves, break-and-restore, and branding the navbar. |
| 0:50 | 20 min | **The two files nobody opens** *(slides 7–8, demo §2)*. `_ViewStart` — why no view asks for a layout. `Layout = null;` on Privacy, View Source, *not even an `<html>` tag*, **restore**. `_ViewImports` — why `asp-controller` resolves, and flag it forward: it's why `<partial />` will work in ten minutes. Short and demystifying. |
| 1:10 | 10 min | **☕ Break** |
| 1:20 | 45 min | **Partials** *(slides 9–13, demo §3)*. **Load-bearing.** Footer → `_Footer.cshtml` → `<partial name="_Footer" />`, and the page looks *identical* — output unchanged, structure changed. Then `_TruckCard.cshtml` with `@model Truck`, rendered in the index loop **and** in an "Also in this city" panel on Details. Finish by editing the card once and refreshing both pages. **Do not rush this.** |
| 2:05 | 10 min | **☕ Break** |
| 2:15 | 20 min | **Sections** *(slides 14–15, demo §4)*. The slot that was always there. Add `@section Scripts`, View Source, find it below the footer and below jQuery. Then `required: true` → index 500s, details still fine. **Restore.** **Say the week-6 line here**, pointing at `_ValidationScriptsPartial.cshtml`. |
| 2:35 | 25 min | **Bootswatch** *(slides 16–18, demo §5)*. **The payoff.** One `<link>`, hard-refresh, different site. Cycle two or three themes. Pinned version, replaces-not-adds, only-the-CSS-moved. Fix the navbar classes, and say the reassurance out loud: *your Bootstrap knowledge is intact*. Close on the week-2 callback. |
| 3:00 | 35 min | **Lab: the Registry gets a shell** *(slide 19)*. Launch with ~90 seconds of *what done looks like* — finished copy + `dotnet test` printing **6 / 6**. Then the setup, said once: **`git pull` → copy `week-05/lab/starter` out and rename → open the folder holding *both* projects → `dotnet test Cryptids.Checks`**. **In-class target: checks 1–4 green.** |
...
- **Someone will push back on "the view renders first."** It sounds backwards, because the finished page has their content nested *inside* the layout. Both are true: the output is nested, the execution is view-then-layout, and they come apart because the view's output is buffered rather than streamed. Order for one request is `_ViewStart` → view → layout → `@RenderBody()`. **Settle it with `ViewData["Title"]` rather than by assertion:** the view sets it, the layout prints it — impossible if the layout had already run. The notes have the full answer; it's worth reading before class because the question is a good one and a hedge here costs you the segment.
...
- **The underscore prefix is convention, not enforcement.** `Footer.cshtml` would render fine. Someone will otherwise believe the framework requires it.
...
- **If time runs short:** the lab can start 10 minutes late and still bank checks 1–3. Slide 20 (the diagram + the week-6 hook) survives anything.
