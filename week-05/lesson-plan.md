...
| 2:50 | 45 min | **Lab: the Registry gets a shell** *(slide 19)*. Launch with ~90 seconds of *what done looks like* — the answer key **running on localhost** + `dotnet test` printing **6 / 6**. **Nothing is deployed for this**; Azure is their homework, not tonight. Then the setup, said once: **`git -C dotnet-web-starters pull` → copy `week-05` out and rename → open the folder holding *both* projects → `dotnet test Cryptids.Checks`**. **In-class target: checks 1–4 green.** |
