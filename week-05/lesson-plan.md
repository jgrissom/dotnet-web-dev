...
| 2:50 | 45 min | **Lab: the Registry gets a shell** *(slide 19)*. Launch with ~90 seconds of *what done looks like* — finished copy + `dotnet test` printing **6 / 6**. Then the setup, said once: **`git pull` → copy `week-05/lab/starter` out and rename → open the folder holding *both* projects → `dotnet test Cryptids.Checks`**. **In-class target: checks 1–4 green.** |
