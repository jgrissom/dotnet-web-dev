...
3. **[The Edit pair, ported](lecture-notes.md#the-edit-pair)** — both `Edit` actions in your real controller, **async**, plus an `Edit.cshtml`. **Take the actions out of your scaffold controller rather than retyping them from the notes** — it already has your property names and your `[Bind]` list, written from your own model, and that is the whole reason you scaffolded. ⚠️ **Three methods cross over, not two** — the POST's `catch` calls a small `private bool ...Exists(int id)` helper the scaffolder left at the bottom of that same file, and it goes away when you delete the scaffold in requirement 5. Without it your project stops compiling. **For the view, start from your own `Create.cshtml`** — it already has your styling, your fields and your validation spans, so the diff is small. ⚠️ **The one everybody misses is the `<form>` tag itself — change `asp-action="Create"` to `asp-action="Edit"`.** Leave it and your Edit page posts to your Create action, which files a *second* record instead of correcting the first. Then relabel the button, send Cancel to Details, and add **[the hidden `Id`](lecture-notes.md#the-hidden-id)**. That last one is the only thing Create never needed, and it's what lets the form carry its own identity — your scaffold's `Edit.cshtml` is the reference if you want to check yourself against it. An **Edit link** on your details page (or your cards) is the way in — [either spelling works](lecture-notes.md#the-way-in): the `href="/Things/Edit/@Model.Id"` you've written since week 4, or the `asp-route-id` tag helper the scaffold uses.
