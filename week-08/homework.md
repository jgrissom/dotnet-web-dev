...
3. **[The Edit pair, ported](lecture-notes.md#the-edit-pair)** — both `Edit` actions, **async**, plus an `Edit.cshtml`.
   ...
   - **The view** — start from your `Create.cshtml`; the styling, fields and validation spans are already right. Change `asp-action="Create"` to **`asp-action="Edit"`** ⚠️ *(the one everybody misses — leave it and Save files a second record instead of correcting the first)*, relabel the button, point Cancel at Details, and add the **hidden `Id`** — the only thing Create never needed.
