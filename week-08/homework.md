...
1. **[The two packages and the tool](lecture-notes.md#two-packages-and-a-tool)** — `Microsoft.VisualStudio.Web.CodeGeneration.Design` and `Microsoft.EntityFrameworkCore.Tools` added to your web project, and `dotnet-aspnet-codegenerator` installed globally. *(The lab handed you the packages; here you run the commands yourself.)* Both come back out in requirement 5, once the scaffold has done its job.
...
3. **[The Edit pair, ported](lecture-notes.md#the-edit-pair)** — both `Edit` actions, **async**, plus an `Edit.cshtml`.
   - **The actions** — copy them from your scaffold controller, not from the notes: it already has your property names and your `[Bind]` list, written from your model. ⚠️ **Three methods cross over, not two** — the `catch` calls a `private bool ...Exists(int id)` helper further down that file, and it goes away when requirement 5 deletes the scaffold.
   - **The view** — start from your `Create.cshtml`; the styling, fields and validation spans are already right. Change `asp-action="Create"` to **`asp-action="Edit"`** ⚠️ *(the one everybody misses — leave it and Save files a second record instead of correcting the first)*, relabel the button, and send Cancel to **this record's** Details page instead of the list — unlike Create's, [that link has to carry the id](lecture-notes.md#the-way-in). Then add the **hidden `Id`** input, which is the only thing Create never needed. *(Two different ids: one is a route value in a link, one is a field inside the form.)*
   - **The way in** — an Edit link on your details page or cards. [Either spelling works](lecture-notes.md#the-way-in): `href="/Things/Edit/@Model.Id"`, or the `asp-route-id` tag helper your scaffold uses.
4. **[The Delete pair, ported](lecture-notes.md#the-delete-pair)** — a GET that shows a confirmation page and changes nothing, a `DeleteConfirmed` POST that deletes, a `Delete.cshtml`, and a Delete link. **Both actions are in your scaffold controller too** — same move as the Edit pair. [Why it's two requests](lecture-notes.md#why-delete-asks-first) is the kind of thing I ask about in person.
5. **The scaffold deleted again** — controller and views both, before you commit. It was a reference. *(Restart after — deleting a class is a rude edit, week 7's lesson.)* Then [put the tool back in the box](lecture-notes.md#and-the-tool-goes-back-in-the-box): `dotnet remove package` for **both** scaffolding packages. The `NU1901` warnings stop when they go, and ⚠️ **`Microsoft.EntityFrameworkCore.Design` stays** — that's what `dotnet ef` runs on, and requirement 6 needs it.
6. **[One new column, added forward](lecture-notes.md#the-additive-migration)** — your model grows **one property of your choosing** (a nullable `string?` is the easy win — a note, a tagline, a location detail; [why nullable](lecture-notes.md#nullable-and-why)). Then two commands, both from inside your web project folder — `dotnet ef migrations add AddWhatever` to write the migration, then `dotnet ef database update` to apply it. **Additive** — your `Migrations` folder only grows now. Seeding values for your existing records with `HasData` is optional but makes your list look finished.
...
- **`The view 'Edit' was not found` — and the error names the exact path your file is at** — look at the `dotnet watch` terminal: a *new* `.cshtml` can't be hot-reloaded, so watch stopped and asked **`Do you want to restart your app? Yes (y) / No (n) / Always (a) / Never (v)`**. Until you answer, the running app is the one built before your file existed. Press **`a`**. **Don't move the file** — it's already in the right place.
- **Saving an edit files a duplicate instead of correcting the record** — two causes, and check them in this order. **Your `<form>` tag still says `asp-action="Create"`** (it will, if you copied `Create.cshtml`), so Save never reaches your Edit action at all. Or [the hidden `Id`](lecture-notes.md#the-hidden-id) is missing and your form's action carries no id either, so `Update()` filed an unset key as a new record. The quiet ones this week, after `[Bind]`.
