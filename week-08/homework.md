...
3. **[The Edit pair, ported](lecture-notes.md#the-edit-pair)** — both `Edit` actions in your real controller, **async**, with an `Edit.cshtml` in your app's own style. **Take them out of your scaffold controller rather than retyping them from the notes** — it already has your property names and your `[Bind]` list, written from your own model, and that is the whole reason you scaffolded. ⚠️ **Three methods cross over, not two** — the POST's `catch` calls a small `private bool ...Exists(int id)` helper the scaffolder left at the bottom of that same file, and it goes away when you delete the scaffold in requirement 5. Without it your project stops compiling. The **[hidden `Id`](lecture-notes.md#the-hidden-id)** is what lets the form carry its own identity, and an **Edit link** on your details page (or your cards) is the way in.
