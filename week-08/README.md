...
- ⚠️ **Copy `week-08/demo-starter/Curbside` out of the private answer-keys repo**, set your secret (`set` only — the id ships), point it at a **fresh database name**, and run `dotnet ef database update` *before* class. The shipped migrations refuse a database that remembers week 7's rehearsal
