...
- [ ] **Set your connection string in your copy** — the `<UserSecretsId>` ships in the `.csproj`, so `set` alone is enough, no `init`:
  ...
  ```
- [ ] ⚠️ **Point `Database=` at a FRESH name** (or drop last week's: `dotnet ef database drop --force`). The shipped migrations are new files with new ids — a database that already has a week-7 `__EFMigrationsHistory` refuses them with *"there is already an object named 'Trucks'"*. Same trap the lab's new database name exists to avoid; better to meet it here than at 2:45
...
- [ ] 🎞️ **GO TO SLIDE 8** — *Task is a Promise* · 🎯 lean on what they know: *"you have written this exact shape for two semesters. `Task<IActionResult>` is `Promise<result>`. `await` is `await`. The method gets marked `async`, the query methods grow an `Async` suffix, and that is the entire mechanical difference"*
...
- [ ] Setup on screen, said once: **`git pull` → copy `week-08/lab/starter` out and rename it → open the folder holding *both* projects → set your connection string with a NEW database name → `dotnet ef database update` → `dotnet test Cryptids.Checks`** → **1 / 6**, and that 1 is the app you were handed
- [ ] ⚠️ **The new database name is not optional, and say why:** *"your week-7 database's migration history is yours — your timestamps, your files. The starter ships mine. They can't mix: pointing the starter at your old database fails with 'there is already an object named Cryptids'. Fresh name — `CryptidsCrud_` yours — and one `database update` builds the whole thing, creatures included. The fact that that works is what a migration IS"*
