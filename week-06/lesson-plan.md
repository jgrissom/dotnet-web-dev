...
- **Curbside**, copied out of the private answer-keys repo (`week-06/demo-starter/Curbside`) into `~/Repos/dotnet-web-dev-course-trial/instructor/`, running under `dotnet watch`, with **two browser tabs parked** on `/Trucks` and `/Trucks/Details/2`
...
| 2:45 | 50 min | **Lab: the Registry takes reports** *(slide 23)*. Launch with ~90 seconds of *what done looks like* — a working form + `dotnet test` printing **6 / 6**. Then the setup, said once: **`cd dotnet-web-starters && git pull` → copy `week-06` out and rename → open the folder holding *both* projects → `dotnet test Cryptids.Checks`**. **In-class target: checks 1–5.** |
