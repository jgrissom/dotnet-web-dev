...
| 0:10 | 30 min | **The round trip** *(slides 3–7, demo §1)*. **Start with the plain HTML form** — no tag helpers, no `@model`, week-2 markup — and a POST action that does nothing but `Content($"You sent: …")`. The whole segment is one moment: *a `Truck` arrived fully populated and I wrote nothing to build it.* Then the Network tab (the payload is week 3's query-string format, in the body), then **break #1**: rename one `name` attribute and watch the property arrive empty with no error. Then **break #2**: delete `[HttpPost]` and the page 500s with `AmbiguousMatchException`. **Both restores are in the script.** |
...
- **`Content()` is doing real work in §1.** No view, no redirect, no validation — just "show me what arrived." It keeps the first ten minutes about *one* idea. Resist the urge to wire up the real action early.
