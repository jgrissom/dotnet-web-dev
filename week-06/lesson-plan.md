...
| 2:45 | 50 min | **Lab: the Registry takes reports** *(slide 23)*. Launch with ~90 seconds of *what done looks like* — the answer key **running on localhost**, form working, + `dotnet test` printing **6 / 6**. **Nothing is deployed for this**; Azure is their homework, not tonight. Then the setup, said once: **`git -C dotnet-web-starters pull` → copy `week-06` out and rename → open the folder holding *both* projects → `dotnet test Cryptids.Checks`**. **In-class target: checks 1–5.** |
