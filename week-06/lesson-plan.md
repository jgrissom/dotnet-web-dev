...
- **The terminal is a display surface in §1, not just where the app runs.** Size it for the back row, and use `clear` (⌘K) deliberately: wipe it before the good submission so one object is on screen, then **leave it alone** through break #1 so the broken result lands directly underneath. The two blocks side by side are the beat — `Content()` could never do that, because it only ever shows the latest submission.
