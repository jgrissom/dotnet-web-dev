...
- **You changed an annotation and the page didn't change** — a data annotation is an attribute, and `dotnet watch` applies an attribute-only edit only *sometimes*. It prints `Hot reload succeeded` either way. Press **`Ctrl+R`** in the terminal running `dotnet watch`. Until you do, your form keeps rendering the *old* `data-val-*` attributes — so the self-check scores rules you already deleted, or misses ones you just added, and you end up rewriting an annotation that was right the first time.
