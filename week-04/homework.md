...
3. **An Index page listing all of them, with a link on every row** to that item's details page. Built with [`@model`](lecture-notes.md#strongly-typed-views-with-model) and [`@foreach`](lecture-notes.md#loops-in-a-view), the link being `href="/Things/Details/@item.Id"` [inside the loop](lecture-notes.md#loops-in-a-view). You'll need a [controller class and a matching `Views/<Name>/` folder](lecture-notes.md#conventions-three-names-that-must-agree) — three names must agree. *(The links are part of this step, not a later one — the self-check counts your items by them.)*
4. **A nav link** to your Index page — do this one **early**. Copy the `Privacy` `<li>` in [`Views/Shared/_Layout.cshtml`](../week-03/lecture-notes.md#project-anatomy) and adapt it. **[The notes write the whole edit out](lecture-notes.md#the-navbar-and-the-one-nav-link-your-homework-needs)** — three things change — and explain the tag helpers you'll see in there, which are fine to leave alone. *(That's the only layout change you need — the shell is week 5's business.)* **The self-check finds your controller by following this link.**
