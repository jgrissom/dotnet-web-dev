...
- `~/Repos/dotnet-web-dev-course/instructor/week-04/` ready and empty for **Curbside**; terminal font sized for the projector
...
| 1:15 | 10 min | **☕ Break** |
| 1:25 | 45 min | **Razor for real** *(slides 8–12, demo §3)*. Expressions → code blocks → `@if` → `@foreach` → comments, with **View Source after every beat**. Load-bearing segment #1: the "one `<li>` in, four out" moment is the thesis of the entire week. Don't rush to the model. |
| 2:10 | 10 min | **☕ Break** |
...
| 2:55 | 40 min | **Lab: Cryptid Registry** *(slide 19)*. Launch with ~90 seconds of *what done looks like*: the answer key **running on localhost** + `dotnet test` printing **6 / 6** — a target, not a walkthrough. **Nothing is deployed for this**; Azure is their homework, not tonight. Then put the three setup steps on screen and say them once, because this is where the room stalls: **`cd dotnet-web-starters && git pull` → copy `week-04` out and rename it → open that folder (the one holding *both* projects) → `dotnet test Cryptids.Checks`**. Nobody creates a project; it already exists. **In-class target: checks 1–4 green.** Checks 5–6 and the deploy roll into homework by design — say so. |
...
- **Show the homework self-checker at the wrap-up**, not just in the handout. Students add one `<script>` tag to their index view — *"exactly like the Bootstrap CDN in week 2, and exactly like week 1's checker"* — then F12 and read the console. Frame it as a **finish line, not a progress bar** — it recognises their work by a nav link, per-row Details links and a working `/Details/1`, so half-built apps honestly score near zero and students who run it early think it's broken (this happened on the first real walkthrough). Tell them: build it, then check it, then deploy and check again. It runs the same checks that earn 12 of the homework's 20 points, needs no install, and deliberately avoids the console-paste route: Chrome and Edge block the first paste until you type `allow pasting`, which would hit every student at once. Mention that the red `404` is on purpose, and that `recheck()` re-runs it without a refresh.
...
- **The homework's nav link (requirement 4) is never demoed — on purpose, and the notes cover it instead.** Adding it means opening `_Layout.cshtml`, and tonight is about proving a URL is an instruction by *typing* it; a nav link hides the URL behind a click, and the shell is week 5's opening move. But it's worth 2 points **and** it's how the self-checker discovers their controller, so **name it at the wrap-up and point at the notes** — the whole edit is written out there, with the three things that change.
