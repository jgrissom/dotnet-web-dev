...
| 0:15 | 40 min | **Routing deep dive** *(slides 3–5, demo §1)*. ⚠️ **Curbside must already be running** — spin it up during the gallery walk (demo §0b: `dotnet new mvc --no-https`, `dotnet watch`). Slide 3 is the pattern slot by slot; **slide 4 is live** — you type each URL in the browser and the room predicts before you press Enter (protect the `/Privacy` beat, it's the one that lands). Slide 5 is where you edit the pattern: move the default action, delete `{id?}`, watch both break, **restore**. Close on 404-vs-500 as a diagnosis habit. |
