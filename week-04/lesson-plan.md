...
- **Show the homework self-checker at the wrap-up**, not just in the handout — open a deployed app, F12, paste the one-liner from the homework, watch it print ✅s. It runs the same checks that earn 12 of the homework's 20 points and needs no install (browser console, same as weeks 1–2). Two things to say while it runs: the red `404` is deliberate, and **Chrome/Edge will demand they type `allow pasting` the first time** — do it on the projector so all of them see it once instead of twenty of them emailing you. The homework has an offline fallback (paste the file itself) if the network fights you.
